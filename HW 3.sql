...
('Sewed the Vest Pocket','1998-09-08','1555 W Lane Ave, Columbus, OH, 43221','country'), 
('Onn & Onn','1989-02-18','11 E Rancho Madera Rd, Madison, WI, 53707','folk'), 
('Burt Ruggles','1999-06-12','60 Madison Ave, New York, NY, 10010','other'), 
('No Rest For The Weary','2004-10-11','2021 K Street Nw, Washington, DC, 20006','jazz'), 
('Umami','1997-11-02','4775 E Miami River Rd, Cleves, OH, 45002','rock'), 
('The Ubernerds','1997-06-23','3250 Spring Grove Ave, Cincinnati, OH, 45225','pop'),
('Jess & Odie', '1981-01-29', '4172 Zaring Mill Rd, Shelbyville, KY, 40065', 'folk');
...
-- you can not delete from a table which is the parent in a relationship
...
SELECT co.customer_first_name, co.customer_last_name, COUNT(DISTINCT co.title) AS unique_item_count
FROM customer_order_view co
GROUP BY co.customer_first_name, co.customer_last_name;

SELECT co.customer_first_name, co.customer_last_name, COUNT(DISTINCT co.title) AS unique_item_count
FROM customer_order_view co
GROUP BY co.customer_first_name, co.customer_last_name
HAVING COUNT(DISTINCT co.title) > (
    SELECT AVG(item_count)
    FROM (SELECT customer_first_name, COUNT(DISTINCT title) AS item_count FROM customer_order_view GROUP BY customer_first_name) AS avg_count
);

SELECT c.customer_zip, COUNT(DISTINCT i.title) AS unique_item_count
FROM customers c
...
SELECT c.customer_state, COUNT(DISTINCT ab.artist_genre) AS unique_genre_count
FROM customers c
LEFT JOIN orders o ON c.customer_id = o.customer_id
LEFT JOIN order_details od ON o.order_id = od.order_id
LEFT JOIN items i ON od.item_id = i.item_id
LEFT JOIN artist_bdwark01 ab ON i.artist = ab.artist_name
GROUP BY c.customer_state;

/*
A. A book can have only one publisher since the Isbn in the BOOK table is a 
	foreign key that references the publisher table's primary key (Name). 
	This is a one-to-one (1-1) relationship.

B. A book can have multiple authors, and an author can write multiple books. 
	This is a many-to-many (many-many) relationship. The Writes table acts 
	as a junction table to represent this relationship.

C. Create the Publisher table. Create the Author table. 
	Create the Book tableFinally, create the Writes table
 */
